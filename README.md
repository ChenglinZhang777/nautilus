# BMAD AgentTeam Framework

基于 Claude Code 的 AI Agent 协作开发框架（BMAD v6.0.3）。

将 Sprint 规划、并行实现、多维 QA 审查、进化记忆等能力封装为可安装的框架，在任意项目目录一键启用。
...
PROJECT_NAME=myapp USER_NAME=Alice LANGUAGE=Chinese \
  bash <(curl -fsSL https://raw.githubusercontent.com/YOUR_ORG/bmad-framework/main/scripts/install.sh)
```
...
```
your-project/
  _bmad/            ← BMAD 框架文件（personas、workflows、执行引擎）
  CLAUDE.md         ← Claude Code 操作规则（自动加载）
  docs/             ← 项目知识库（可随时添加文档）
  .gitignore        ← 自动追加 BMAD 运行时数据忽略规则
```
...
重新运行安装脚本即可更新框架文件（`_bmad/` 会被覆盖，`CLAUDE.md` 备份为 `.bak`）：
...
```bash
git clone https://github.com/YOUR_ORG/bmad-framework
cd your-project
PROJECT_NAME=myapp USER_NAME=Alice LANGUAGE=Chinese \
